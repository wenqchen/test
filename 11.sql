...
WHERE ad.SRC_CRE_DT BETWEEN '2021-04-01' AND CURRENT_DATE   /*use hst.AD_STATUS_START_DATE instead of ad.SRC_CRE_DT*/
...
WHERE REPLY.CLSFD_SITE_ID IN (9021)
    AND REPLY.SRC_CRE_DT BETWEEN '2021-04-01' AND CURRENT_DATE   /*use hst.AD_STATUS_START_DATE instead of ad.SRC_CRE_DT*/
...
select
        ...
        clsfd_event_label,
      CASE 
            WHEN lower(clsfd_event_label)=lower('sold_on_ebayk') or lower(clsfd_event_label)=lower('soldonebayk') or lower(clsfd_event_label)=lower('SoldAtEBayK') or lower(clsfd_event_label)=lower('sold;SoldAtEBayK') or lower(clsfd_event_label)=lower('delete;SoldAtEBayK') or lower(clsfd_event_label)=lower('DeleteAd;SoldAtEBayK') THEN 1 
            WHEN lower(clsfd_event_label)=lower('sold_elsewhere') or lower(clsfd_event_label)=lower('soldsomewhereelse') or lower(clsfd_event_label)=lower('sold;SoldSomewhereElse')  or lower(clsfd_event_label)=lower('delete;SoldSomewhereElse') or lower(clsfd_event_label)=lower('DeleteAd;SoldSomewhereElse') THEN 0 
...
WHERE    ecg_session_start_dt BETWEEN '2021-04-01' AND CURRENT_DATE   /*use hst.AD_STATUS_START_DATE instead of ad.SRC_CRE_DT*/
...
insert overwrite table clsfd_working.clsfd_prdct_trans_src partition(clsfd_site_id)
 select
    clsfd_ad_id as clsfd_ad_id,
    ad_starttime as ad_strt_tm,
    seller_tenure as slr_tenure_tm,
    numconversations as convr_cnt,
    numconversationswithconnection as convr_with_conn_cnt,
    nummessagestotal as msg_cnt,
    time_ad_post_until_first_connection as ad_post_until_first_conn_tm,
    ask_price_lc as ask_price_amt,
    nummessagesseller as  slr_msg_cnt,
    sold_on_ebayk as sold_flag,
    null as pymnt_flag,
    CURRENT_DATE(),
    CURRENT_USER(),
    NULL,
